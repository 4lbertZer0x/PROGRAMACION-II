...
export class PaymentsModel {
    ...
    public async create(payment: Payment): Promise<boolean> {
        return new Promise((resolve, reject) => {
            const { email, cardholder_name, amount, currency, service_type, status, transaction_id, created_at } = payment;
            const createdAtValue = created_at || new Date().toISOString();
            const query = `
                INSERT INTO payments (email, cardholder_name, amount, currency, service_type, status, transaction_id, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `;

            this.db.run(
                query,
                [email, cardholder_name, amount, currency, service_type, status, transaction_id, createdAtValue],
                (err) => {
                    if (err) {
