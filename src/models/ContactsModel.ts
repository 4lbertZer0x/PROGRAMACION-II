...
export class ContactsModel {
    ...
    public async create(contact: Contact): Promise<boolean> {
        return new Promise((resolve, reject) => {
            const { email, name, comment, ip_address, service, created_at } = contact;
            const createdAtValue = created_at || new Date().toISOString();
            const query = `
                INSERT INTO contacts (email, name, comment, ip_address, service, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
            `;

            this.db.run(query, [email, name, comment, ip_address, service || '', createdAtValue], (err) => {
                if (err) {
